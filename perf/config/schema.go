...
func (sc *ScenarioConfig) Validate(name string) error {
	if sc.Executor == "" {
		sc.Executor = "constant-vus" // Default executor
	}
	...
	if !validExecutors[sc.Executor] {
		return fmt.Errorf("unknown executor type: %s", sc.Executor)
	}
	...
	case "constant-vus":
		...
		if sc.Duration == "" {
			return fmt.Errorf("duration is required for constant-vus executor")
		}

	case "ramping-vus":
		if len(sc.Stages) == 0 {
			return fmt.Errorf("stages are required for ramping-vus executor")
		}

	case "constant-arrival-rate":
		if sc.Rate <= 0 {
			return fmt.Errorf("rate must be > 0 for constant-arrival-rate executor")
		}
		if sc.Duration == "" {
			return fmt.Errorf("duration is required for constant-arrival-rate executor")
		}

	case "ramping-arrival-rate":
		if len(sc.Stages) == 0 {
			return fmt.Errorf("stages are required for ramping-arrival-rate executor")
		}
	}
	...
	for i, req := range sc.Requests {
		...
		if req.Method == "" {
			sc.Requests[i].Method = "GET" // Default method
		}
	}
