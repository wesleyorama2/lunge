...
func (lb *LeakyBucket) Next() time.Time {
	...
	// Set lastDrip to nextTime to prevent double-counting when we wake up
...
// Stats contains statistics about the leaky bucket.
type Stats struct {
	// Rate is the current rate in iterations/second
	Rate float64 `json:"rate"`

	// Accumulated is the currently accumulated iterations (fractional)
	Accumulated float64 `json:"accumulated"`

	// MaxBurst is the maximum burst size
	MaxBurst float64 `json:"maxBurst"`

	// TotalIterations is the total iterations scheduled
	TotalIterations int64 `json:"totalIterations"`

	// TotalWaitTime is the total time spent waiting
